        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    @guest
                    @else
                        <x-nav-link :href="route('home')" :active="request()->routeIs('home')" class="font-medium">
                            {{ __('Hovedmenu') }}
                        </x-nav-link>
                        <x-nav-link :href="route('credentials.index')" :active="request()->routeIs('credentials.index')" class="font-medium">
                            {{ __('Key store') }}
                        </x-nav-link>
                        @if(auth()->user()->role == \App\Enums\UserRoleEnum::Admin)
                            <x-nav-link :href="route('admin.users.index')" :active="request()->routeIs('admin.users.index')" class="font-medium">
                                {{__('app.user_list')}}
                            </x-nav-link>
                        @endif
                    ...
                    <x-slot name="content">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            ...
                            <x-dropdown-link :href="route('logout')"
                                            ...
                                            this.closest('form').submit();" class="font-medium">
                                {{ __('Log Out') }}
                            </x-dropdown-link>
                        </form>
                    </x-slot>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4">
                ...
                <div class="font-medium text-sm text-gray-500">{{ Auth::check() ? Auth::user()->name : '' }}</div>
            </div>
